...
extern sl_server_queue_t g_server_queue_pool[];
...
int sl_server_queue_init(sl_server_t *server, int size)
{
    ...
    return g_server_queue_pool[server->queue_type].init(size);
}
...
int sl_server_queue_destroy(sl_server_t *server)
{
    ...
    return g_server_queue_pool[server->queue_type].destroy();
}
...
int sl_server_queue_put_item(sl_server_t *server, int item)
{
    ...
    while (g_server_queue_pool[server->queue_type].full())
	pthread_cond_wait(&g_shared.empty, &g_shared.mutex);

    g_server_queue_pool[server->queue_type].put(item);
...
int sl_server_queue_get_item(sl_server_t *server, int *item)
{
    ...
    while (g_server_queue_pool[server->queue_type].empty())
	pthread_cond_wait(&g_shared.full, &g_shared.mutex);
 
    g_server_queue_pool[server->queue_type].get(item);
